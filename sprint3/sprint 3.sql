...
WHERE EXTRACT(YEAR FROM movimiento.timestamp) = 2021 AND declined = 0
...
SELECT
    ...
    ROUND (CAST(REGEXP_REPLACE(CAST(price AS STRING), r'[$,ۣ ]', '') AS FLOAT64),2) AS price,
    colour,
    weight,
...
LEFT JOIN -- Hacemos un left join para asegurarnos que esten todos los productos los que tienen y los que no tienen transanciones.
  (
    SELECT 
      transaction_id,
      TRIM(product_id) AS product_id_flat
    FROM
      `sprint3-analytics-osley.sprint3_silver.transactions_clean`,
      --  UNNEST(SPLIT(product_ids, ',')) AS product_id convertimos la canedna de testo separada por comas en filas individuales
