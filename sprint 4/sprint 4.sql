...
SELECT
  * EXCEPT(timestamp),
  ...
  -- RAND() * 50 genera un nuemro aleatorio entre 0 y 50, con CAST lo convertimos en entero, con TIMESTAMP_SUB se lo rstamos al tiempo actual CURRENT_TIMESTAMP
FROM
...
SELECT 
      dia AS fecha,
      total AS ventas_hoy,
      LAG(total) OVER (ORDER BY dia) AS ventas_ayer,
      ROUND ((total - LAG(total) OVER (ORDER BY dia))*100/total,2) AS Diff_Percentual
FROM `sprint3-analytics-osley.sprint3_gold.mv_daily_sales`
ORDER BY fecha DESC;
...
/*Ejercicio 1, Desanimiento y Allanamiento de Datos (Unnesting)*/
